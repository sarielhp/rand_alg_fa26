%-----------------------------------------------------------------------------
% Master Prefix File for Lwarp (HTML Web Generation)
% Compatible with LuaLaTeX and Lwarp
%-----------------------------------------------------------------------------
...
\ifLuaTeX
  % LuaLaTeX verified
\else
  \PackageError{lwarp-gate}{%
    prefix_lwarp requires LuaLaTeX! Please run with lualatex.%
  }{HTML generation via lwarp must be run with LuaLaTeX.}
  \stop
\fi
...
% Load lwarp immediately after documentclass
\usepackage[mathjax]{lwarp}

% Lwarp settings: split at chapter level (0) with section names
\setcounter{FileDepth}{0}
\booltrue{FileSectionNames}
\setcounter{SideTOCDepth}{1}

\makeatletter
% Multi-page SVG resolving hook for Ipe drawings with page=N
\def\LWR@current@figpage{}
\define@key{igraph}{page}{\def\LWR@current@figpage{#1}}
% Perpage compatibility for auxiliary files
...
\AtBeginDocument{
  \ifdef{\LWR@nullfonts}{%
    \appto\LWR@nullfonts{%
      ...
      \def\Erdos{Erdős}%
      \def\Holder{Hölder}%
      \def\Matousek{Matoušek}%
    }%
  }{}%
  \ifdef{\LWR@replaceEPSSVG}{
    \appto\LWR@replaceEPSSVG{%
      \ifx\LWR@current@figpage\@empty\else
        \edef\LWR@tempone{\LWR@tempone_p\LWR@current@figpage}%
        \def\LWR@current@figpage{}%
      \fi
    }%
  }{}%
...
\IfFileExists{styles/qed_duck.svg}{%
   \providecommand{\myqedsymbol}{%
      \reflectbox{\includegraphics[height=1.4\CharHeight]{styles/qed_duck}}%
   }%
}{%
...
\newcommand{\FMS}{\EuScript{X}}  
\newcommand{\FMSB}{\EuScript{Y}}  
...
\newcommand{\ballsY}[2]{\mathcal{F}_{\geq #1}\pth{#2}}
...
\DeclareRobustCommand{\Matousek}{Matoušek\xspace}
\DeclareRobustCommand{\Erdos}{Erdős\xspace}
\DeclareRobustCommand{\Lovasz}{Lovász\xspace}
...
\newtheorem*{remarkunnumbered}{Remark}%
\newenvironment{remark_unnumbered}{\begin{remarkunnumbered}}{\end{remarkunnumbered}}
...
\newtheorem{exerciseh}[theorem]{Exercise}
\newenvironment{exercise_h}[1][]{\begin{exerciseh}[#1]}{\end{exerciseh}}
...
\DeclareRobustCommand{\Holder}{Hölder\xspace}
...
\DeclareRobustCommand{\Turan}{Turán\xspace}
