%-----------------------------------------------------------------------------
% TeX4ht Prefix File for Randomized Algorithms Class Notes
% Kept separate from prefix_latex.tex so HTML-specific simplifications cannot
% affect the standard PDF or lwarp builds.
%-----------------------------------------------------------------------------
...
\ifdefined\HCode\else
  \PackageError{tex4ht-gate}{%
    Attempting to use prefix_4ht.tex outside TeX4ht.%
  }{Compile through make4ht, or use prefix_latex.tex for PDF generation.}
  \stop
\fi
...
% QED Duck Symbol
%-----------------------------------------------------------------------------
\IfFileExists{styles/qed_duck.svg}{%
   \providecommand{\myqedsymbol}{%
      \HCode{<img class="qed-duck" src="styles/qed_duck.svg" alt="QED" />}%
   }%
}{%
   \providecommand{\myqedsymbol}{\HCode{<span class="qed-fallback">&#x25A0;</span>}}%
}
...
% Fragments & Quotes
%-----------------------------------------------------------------------------
% The PDF build writes fragment bodies to shared files and later includes them
% in standalone appendices.  A chapter webpage already contains each fragment
% at its original location, so TeX4ht only needs a transparent wrapper and can
% omit the repeated standalone copies.
...
\newcommand{\WebChapterQuote}[3]{%
   \ifvmode\IgnorePar\fi\EndP%
   \HCode{<figure class="chapter-quote"><blockquote>}%
      #1%
   \HCode{</blockquote><hr /><figcaption><span class="quote-author">}%
      #2%
   \HCode{</span><span class="quote-work">}%
      #3%
   \HCode{</span></figcaption></figure>}%
   \par%
}

\newcommand{\QuotePExt}[3]{\WebChapterQuote{#1}{#2}{#3}}

\epigraphfontsize{\small\itshape}
\setlength{\epigraphwidth}{0.6\textwidth}
\newcommand{\QuoteExt}[3]{\WebChapterQuote{#1}{#2}{#3}}
...
% PicTeX's tree macros perform box arithmetic that TeX4ht cannot convert and
% can terminate the HTML document at the first tree.  These lightweight web
% forms preserve the two children and optional subtree label as HTML/CSS.
\renewcommand{\tree}[2]{%
   \HCode{<span class="web-binary-tree"><span class="tree-node"></span><span class="tree-children"><span class="tree-child">}%
   #1%
   \HCode{</span><span class="tree-child">}%
   #2%
   \HCode{</span></span></span>}%
}
\renewcommand{\TreePKGtriangle}[3][]{%
   \HCode{<span class="web-subtree-triangle"><span class="subtree-label">}%
   #1%
   \HCode{</span></span>}%
}
...
\newcommand{\pntCoord}{p}
\newcommand{\pntCoordA}{q}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Markov chain notations.

\newcommand{\MCSpc}{\mathtt{S}}
\newcommand{\MCPrb}{\mathtt{P}}
\newcommand{\MCPrbY}[2]{\MCPrb_{#1#2}}
\newcommand{\MCPrbM}{\mathbf{P}}%
\newcommand{\MCPrbT}[3]{\MCPrb_{#1#2}^{(#3)}}
\newcommand{\MCFPrbT}[3]{\mathtt{r}_{#1#2}^{(#3)}}
\newcommand{\MCVstProb}[2]{\mathtt{f}_{#1#2}}
\newcommand{\MCExSteps}[2]{\mathtt{h}_{#1#2}}
...
\newcommand{\medX}[1]{\mathrm{med}\pth{#1}}
...
\newcommand{\Event}{\EuScript{E}}%
\newcommand{\EventB}{\EuScript{B}}%
\newcommand{\EventF}{\EuScript{F}}%
\newcommand{\EventG}{\EuScript{G}}%
...
\newcommand{\probref}[1]{\HLink{Problem}{prob:#1}}%
\newcommand{\probrefY}[2]{\hyperref[prob:#1]{#2}}
...
\theoremseparator{}
% TeX4ht's ntheorem support consumes theorem end symbols.  The web-only
% wrappers below place the duck before ntheorem closes its HTML block.
\theoremsymbol{}
...
\let\endproof@theorem\endproof
\renewenvironment{proof}{\proof@theorem}{\unskip\myqedsymbol\endproof@theorem}
\let\proofextin@theorem\proofextin
\let\endproofextin@theorem\endproofextin
\renewenvironment{proofextin}{\proofextin@theorem}{\unskip\myqedsymbol\endproofextin@theorem}
\let\altproof@theorem\altproof
\let\endaltproof@theorem\endaltproof
\renewenvironment{altproof}{\altproof@theorem}{\unskip\myqedsymbol\endaltproof@theorem}
\let\proofof@theorem\proofof
\let\endproofof@theorem\endproofof
\renewenvironment{proofof}{\proofof@theorem}{\unskip\myqedsymbol\endproofof@theorem}
\makeatother
...
\newcommand{\HSetY}[2]{H\pth{#1, #2}}%
\newcommand{\CHSetY}[2]{C\pth{#1, #2}}%
...
\newcommand{\DA}{D}%
\newcommand{\uniqX}[1]{\mathrm{uniq}\pth{#1}}%
\newcommand{\diffX}[1]{\mathop{}\!\mathrm{d}#1}

\newcommand{\Frq}{F}%
\newcommand{\frq}{f}%
\newcommand{\STRM}{\mathcal{S}}%
\newcommand{\strm}{{s}}%

\providecommand{\Turan}{Tur\'an\xspace}
\newcommand{\fGrowX}[1]{G^{}_{\Dim}\pth{#1}}
...
% Page-breaking hints have no meaning in HTML.  Display bodies are preserved
% verbatim for MathJax, so the builder also removes occurrences found there.
\renewcommand{\allowdisplaybreaks}[1][]{}
